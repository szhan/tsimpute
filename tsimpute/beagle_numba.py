...
# Helper functions.
def is_data_compatible(ref_h, query_h):
    """
    ...
    :rtype: bool
    """
    num_sites = ref_h.shape[0]
    if num_sites != len(query_h):
        raise ValueError("Incompatible reference and query haplotypes.")
    for i in range(num_sites):
        if query_h[i] == tskit.MISSING_DATA:
            continue
        if ~np.any(ref_h[i, :] == query_h[i]):
            # f"Allele {query_h[i]} at {i}-th position is not in reference."
            return False
    return True
...
def convert_to_cm(site_pos, genetic_map=None):
    ...
    num_sites = len(site_pos)
    est_cm = np.zeros(num_sites, dtype=np.float64)  # BEAGLE 4.1 uses double in Java.
    for i in range(num_sites):
...
def get_transition_probs(cm, h, ne):
    """
    Compute transition probabilities to a random state at genotyped positions.
    ...
    num_sites = len(cm)
    r = np.zeros(num_sites, dtype=np.float64)
    r[1:] = np.diff(cm)
    c = -0.04 * (ne / h)
...
def compute_forward_matrix_beaglelike(
    ...
    if len(query_h) != num_sites:
        raise ValueError("Incompatible query haplotype.")
    fwd_mat = np.zeros((num_sites, num_haps), dtype=np.float64)
    ...
    for i in range(num_sites):
        ...
        last_sum = site_sum / num_haps if i == 0 else site_sum
    return fwd_mat
...
def compute_backward_matrix_beaglelike(
    ...
    if len(query_h) != num_sites:
        raise ValueError("Incompatible query haplotype.")
    bwd_mat = np.zeros((num_sites, num_haps), dtype=np.float64)
    bwd_mat[-1, :] = 1.0 / num_haps
    for i in range(num_sites - 2, -1, -1):
        ...
        shift = trans_probs[iP1] / num_haps
...
def compute_state_prob_matrix(fwd_mat, bwd_mat):
    ...
    :rtype: numpy.ndarray
    """
    if fwd_mat.shape != bwd_mat.shape:
        raise ValueError("Incompatible forward and backward matrices.")
    num_sites, num_haps = fwd_mat.shape
...
def interpolate_allele_probs(
    ...
    *,
    ...
    probs = np.zeros((x, num_alleles), dtype=np.float64)
    for i in range(x):
        k = right_idx[i]
        w = weights[i]
        for a in alleles:
    ...
    if ~np.all(site_sums > 0):
        raise ValueError("Site sums of allele probabilities is <= 0.")
...
def get_map_alleles(allele_probs, num_alleles):
    ...
    if a != num_alleles:
        raise ValueError("Unexpected number of alleles.")
...
def run_interpolation_beagle(
    ...
    *,
    ...
    :param numpy.ndarray ref_h: Reference haplotypes.
    :param numpy.ndarray query_h: A query haplotype.
    :param numpy.ndarray pos_all: Site positions of all the markers (bp).
